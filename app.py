...
def run_evaluation(model, labels: dict, class_root: str):
    ...
    """
    # Reverse mapping: class name -> class index, so folder names can be matched
...
def compute_metrics(y_true, y_pred, y_probs, num_classes: int):
    ...
    y_probs_arr = np.array(y_probs)  # shape: (num_samples, num_classes)

    # Cross-entropy loss: compares predicted probability distributions against
    # the true integer class labels. Using Sparse variant since y_true is a
    # list of integer indices, not one-hot encoded vectors.
...
# ===================== PREDICT TAB (original app behavior) =====================
with predict_tab:
...
with evaluate_tab:
    ...
    if uploaded_zip is not None:
        if st.button("Run Evaluation"):
            ...
            # Clean up extracted files now that predictions are collected
